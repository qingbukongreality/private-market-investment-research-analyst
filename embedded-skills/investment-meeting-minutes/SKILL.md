...
- **Source-priority iron rule:** build the topical body primarily from what was actually said in the meeting. The audio and corrected/original transcript determine the factual storyline, emphasis, stage, figures, qualifications and omissions. BP, company introduction and supplemental materials are secondary: use them to correct spelling and terminology and to add limited factual background only. They may not replace, upgrade, embellish or contradict the meeting account.
- A fact appearing only in the BP must not be presented as though it was confirmed in the meeting. Include BP-only background only when it is necessary to understand the discussion, non-promotional, and clearly framed as company-provided information. Exclude BP-only claims about leadership, uniqueness, market share, customer recognition, orders, performance, forecasts or technical superiority unless the meeting independently supports them.
...
- If transcript and BP conflict on a substantive fact, number or stage, the meeting memo must follow and accurately qualify the meeting account. Do not average, merge or silently replace it with the BP version. Preserve the discrepancy for downstream project-intake notes.
...
- For long interviews, Q&A must be generated chunk by chunk along transcript boundaries and concatenated in order. In App mode, target about 3,000 Chinese characters per chunk, splitting at the nearest speaker, timestamp or paragraph boundary. A single full-transcript request is not an acceptable completeness strategy merely because the model context window is large.
- Do not impose a mandatory character-retention ratio and do not automatically issue a second completion request solely because a chunk is short. Completeness comes from processing every source chunk and preserving its substantive questions and answers, not from padding toward a length target.
